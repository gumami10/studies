...
export function transformNode(node) {
  ...
  if (tag === 'key-box') {
    const parts = node.children.filter(c => c.type === 'element').map(transformNode).filter(Boolean)
    const heading = parts.find(p => p.type === 'heading')
    return { type: 'key-box', heading: heading?.text, content: heading ? parts.filter(p => p !== heading) : parts }
  }
  ...
  if (tag === 'compare') {
    const cards = node.children
      ...
      .map(card => ({
        ...
        content: card.children.filter(ch => ch.type === 'element').map(transformNode).filter(Boolean)
      }))
  ...
  if (tag === 'footer-text') return { type: 'footer-text', text: node.children.map(transformNode).join('').trim() }
